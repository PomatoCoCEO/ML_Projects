function train_all()
    ...
    for i = 1:length(act_funct_strs)
        ...
        save("1layer_C_"+act_funct_str+".mat", "one_layer_classifier");
    ...
    for i = 1:length(act_funct_strs)
        ...
        save("1layer_"+act_funct_str+".mat", "one_layer");
        real_outputs = sim(one_layer, test_input);
        % size(real_outputs)
        % convert output for classification evaluation: use convert_output
        [c,cm,ind,per] = confusion(test_lbls_bin, real_outputs);
        fprintf("Classifier with one layer and activation function %s \n" , act_funct_str);
        fprintf("Accuracy: %f\n", (1-c));
        fprintf("Confusion matrix: \n");
        disp(cm);
        fprintf("\n");
    ...
    for i = 1:length(act_funct_strs)
        act_funct_str = act_funct_strs{i};
        two_layer = classifier_two_layers(P_total, labels_binary_total, act_funct_str);
        
        save("2layer_"+act_funct_str+".mat", "two_layer");
    ...
    save("Pattern.mat", "pn");
    real_outputs_pn = sim(pn, test_input);
    % size(real_outputs_pn)
    [c_pn,cm_pn,ind_pn,per_pn] = confusion(test_lbls_bin, real_outputs_pn);
    fprintf("Pattern network\n")
        fprintf("Accuracy: %f\n", (1-c));
        fprintf("Confusion matrix: \n");
        disp(cm);
fprintf("\n");
